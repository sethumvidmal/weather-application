...
function initializeEventListeners() {
    ...
    // Search button click
    elements.searchBtn.addEventListener('click', searchBtnOnClick);
...
async function forecastWeather(location) {
    try {
        const response = await fetch(`https://api.weatherapi.com/v1/forecast.json?q=${location}&days=3&key=${API_KEY}`);
        const data = await response.json();
...
function updateForecast(forecastData) {
    elements.forecastContainer.innerHTML = forecastData.map(day => createForecastCard(day)).join('');
}
